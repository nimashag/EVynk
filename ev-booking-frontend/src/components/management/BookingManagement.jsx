...
import { Calendar, Clock, MapPin, User, Zap, Plus, X, CheckCircle, XCircle, Activity, Edit2, Trash2, Play, CheckSquare, Filter, Search } from 'lucide-react';
...
const BookingManagement = () => {
  ...
  const [deleteConfirm, setDeleteConfirm] = useState({ show: false, booking: null });
  ...
  const handleSubmit = async (e) => {
    ...
    if (reservationDateTime > sevenDaysFromNow) {
      showToast('Reservation date cannot be more than 7 days from now', 'error');
      return;
    }
  ...
  const handleNextStatus = async (booking) => {
    try {
      ...
      if (result.success) {
        showToast(`Booking ${booking.status === 1 ? 'activated' : 'completed'} successfully`, 'success');
        fetchData();
      } else {
        showToast(result.error, 'error');
      }
  ...
  const filteredBookings = [...bookings].reverse().filter(booking => {
    ...
    const matchesFilter = filterStatus === 'all' || booking.status === parseInt(filterStatus);
  ...
  const stats = {
    ...
    const reservationDate = new Date(booking.reservationAtUtc).toLocaleDateString('en-US', {
      year: 'numeric',
      month: 'short',
      day: 'numeric'
    });
    const reservationTime = new Date(booking.reservationAtUtc).toLocaleTimeString('en-US', {
      hour: '2-digit',
      minute: '2-digit'
    });
        ...
        <div className="p-6">
          <div className="flex items-start justify-between mb-4">
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1 min-w-0">
                <h3 className="text-lg font-bold text-gray-900">{owner ? owner.fullName : 'Unknown Owner'}</h3>
          ...
          <div className="space-y-3">
            <div className="flex items-start gap-2 text-sm text-gray-600">
              ...
              <div>
                <span className="font-medium text-gray-900">{station ? station.location : 'Unknown Station'}</span>
          ...
          <div className="flex gap-2 pt-4 mt-4 border-t border-gray-100">
            ...
            {(booking.status === 1 || booking.status === 2) && (
              ...
              >
                {booking.status === 1 ? <Play size={16} /> : <CheckSquare size={16} />}
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
            <div>
              ...
              <p className="text-teal-100 mt-2 text-sm">Manage and monitor all charging bookings</p>
            </div>
            <button
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4 mt-8">
            <div className="bg-white/10 backdrop-blur-sm rounded-xl p-4 border border-white/20">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-teal-200 text-sm font-medium">Total Bookings</p>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-teal-200">
                          {editingBooking ? 'Update booking information' : 'Schedule a new charging session'}
                        </p>
                      </div>
                    </div>
                    <button
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-4">
                      ...
                      >
                        ...
                        {stations.filter(station => station.isActive && station.availableSlots > 0).map(station => (
                          <option key={station.id} value={station.id}>
                            {station.location} - {station.type === 2 ? 'DC' : 'AC'} ({station.availableSlots} slots)
                          </option>
                        ))}
                      </select>
                    </div>
                  ...
                  <div className="bg-teal-50 border border-teal-200 rounded-lg p-4">
                    <p className="text-sm text-teal-800">
                      <strong>Note:</strong> Bookings can be made up to 7 days in advance. Changes or cancellations must be made at least 12 hours before the reservation time.
                    </p>
                  </div>
                ...
                <div className="p-6">
                  <div className="mb-6">
                    <p className="text-gray-700 text-base">
                      Are you sure you want to cancel this booking? The owner will be notified and the slot will become available again.
                    </p>
                  </div>
